...
- The social card's eyebrow reads `CAMREX/ESRI-GIS-RESOURCES` rather than the bare repository name, so a card that travels away from the post it was attached to still says whose it is.
- Added `tools/make_card_bg.py`, which puts a map behind the social card: the lower 48 in Lambert Conformal Conic, fading in from the left, with a pin over Miami carrying a real conversion — E 922,162.27 / N 519,888.66 in EPSG:2236 (NAD83 / Florida East ftUS) resolving to 25.761680°, −80.191790°. Those are not illustrative numbers; arcpy agrees on the forward projection to 0.00 ftUS, and the shipped condensed build returns `25.76167999, -80.191789987` for that easting and northing, which is the printed pair at six decimals. The card's third figure moved to the footer to make room, and the callout is sized to stay legible at the ~552px render a LinkedIn feed uses.
