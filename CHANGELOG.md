...
- Added a byline and publication date to the Arcade landing page — "By Cameron Rex, RMI Valuation LLC · Published 16 August 2026", the date the resource first went public — alongside the machine-readable equivalents: `author` and `article:published_time` meta tags, and a schema.org `TechArticle` block carrying the author, affiliation, date and licence.
- Added a map behind the Arcade landing page's heading, matching the social card: the same outlines, the same Lambert Conformal Conic framing, and the same pinned conversion — E 922,162.27 / N 519,888.66 in EPSG:2236 becoming 25.761680°, −80.191790° — read from the same constants the card uses, so the two cannot drift apart. Generated by `tools/make_map_svg.py` and spliced into `index.html` as inline SVG. Markup rather than an image, so it costs no request, needs no second asset for dark mode — its colours come from the page's own custom properties — and stays crisp at any size. Decoration only: `aria-hidden`, inert to the pointer, clipped by the header so it cannot widen the page, and hidden below 46rem. The page eyebrow now reads `camrex/esri-gis-resources` and links to the repository.
